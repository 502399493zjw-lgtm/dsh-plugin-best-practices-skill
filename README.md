...
DSH 核心在这套流程里是固定版本的目标运行时，不是默认修改对象。插件拥有自己的项目、依赖、测试、版本和发布边界；`main` 保留稳定基线，存在并行任务、未提交改动或 PR 交付时，再为当前任务建立分支/worktree。

```text
DSH 核心（固定的 stock rc.8 运行时，不直接改源码）
                         │
                         ▼
独立插件项目 ── main：稳定、已验证的基线
                         │
                         ├─ 简单且已有干净专用工作区：直接在任务分支开发
                         └─ 并行/脏工作区/PR：一个任务一个 branch + worktree
                                                   │
                                                   ▼
                                      契约与用例 → 实现 → 测试
                                                   │
                                                   ▼
                                      build → verify → pack tarball
                                                   │
                                                   ▼
                                  隔离的 stock DSH rc.8 profile
                                     │             │             │
                                     │             │             └─ 需要时录 GIF
                                     │             │                展示价值/辅助验收
                                     │             │
                                     │             └─ 现场预览
                                     │                localhost URL → 用户亲手操作
                                     │
                                     └─ 自动 smoke/probe
                                        result + provenance → 证明兼容主张
                                                   │
                                                   ▼
                                  敏感扫描 → PR → CI → 授权后的后续动作
                                                   │
                                                   ▼
                                      按 owner/runId 清理临时资源
```

这层项目与 Git 流程不是仪式：直接在 DSH 源码里开发，会把核心改动、插件改动和生成物混在一起，最终无法证明插件能被未修改的 stock DSH 安装；没有独立插件项目，就没有清晰的依赖、tarball 和发布边界；没有稳定 `main` 与按需 worktree，并行 Agent 容易互相覆盖状态、复用错误产物或把无关改动带进 PR。

### 用户如何亲自查看 DSH 配上插件后的效果

用户要求现场查看时，Agent 应基于**目标代码树**打包插件，而不是直接运行某个未记录的源码目录；进入 GitHub 交付时必须对应目标 commit，本地未提交预览则记录 commit + dirty/diff 来源：

1. 将目标代码树打成 npm tarball，并完成包校验。
2. 用核验过的 stock DSH `0.1.0-rc.8`，把 tarball 安装到独立的临时 `DSH_HOME` 和 `web` profile。
3. 仅监听 `127.0.0.1` 的空闲端口启动 DSH Web，把 `http://127.0.0.1:<port>/`、commit/dirty 状态、tarball、DSH 版本及 real/mock 范围交给用户。
4. 用户在浏览器中打开该地址，亲手操作真实的 DSH + 插件。Host-only 插件没有可视界面时，改为交付可查看的配置、状态/API probe 或 CLI 行为，不制造 mock 页面冒充插件效果。
5. 用户确认看完或到达约定超时后，Agent 停止进程并只清理本次 runId 拥有的 profile、端口和临时文件。需要保留时，先报告保留范围；需要让另一台机器访问时，必须另行获得暴露端口、部署或隧道授权。

现场预览、自动 smoke 和 GIF 互相补充但不能互相替代：现场预览让用户自由探索，smoke 用断言与 provenance 证明可复验结论，GIF 则方便在 README/PR 中异步表达功能价值。
